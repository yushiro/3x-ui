...
install_base() {
    case "${release}" in
        ...
        fedora | amzn | virtuozzo | rhel | almalinux | rocky | ol)
            dnf makecache -y && dnf install -y -q cronie curl tar unzip tzdata socat ca-certificates openssl
            ;;
        centos)
            if [[ "${VERSION_ID}" =~ ^7 ]]; then
                yum makecache -y && yum install -y cronie curl tar unzip tzdata socat ca-certificates openssl
            else
                dnf makecache -y && dnf install -y -q cronie curl tar unzip tzdata socat ca-certificates openssl
            fi
            ;;
        arch | manjaro | parch)
            pacman -Sy --noconfirm cronie curl tar unzip tzdata socat ca-certificates openssl
            ;;
...
install_x-ui() {
    ...
    read -r snell_url snell_sha < <(snell_artifact_for_arch "$(arch)") || {
        echo -e "${red}Unsupported Snell CPU architecture${plain}"
        exit 1
    }
    if ! snell_download_and_install "${snell_url}" "${snell_sha}" "${xui_folder}/bin/snell"; then
        echo -e "${red}Failed to install verified Snell v5.0.1${plain}"
        exit 1
    fi
